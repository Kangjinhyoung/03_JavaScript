/* JS 객체 생성 + 다루기
  - JS 객체 : {K:V, K:V, K:V, ... }
  - (중요!!) K(key)는 무조건 string 이다
    -> "Key" 또는 'Key' 또는 Key  
       => "", '' 붙이지 않아도 알아서 string으로 해석한다
  - JS 객체를 콘솔에 출력하는 경우
...
btn2.addEventListener("click", () => {
  ...
  /* 
    this 참조 변수 
    - 작성된 위치마다 의미가 다르다
    1) 객체 내 함수(메서드) this == 같은 객체(여기, 이 객체)
...
addStudent.addEventListener("click", () => {
  ...
  // 생선된 Student 객체를 studentList에 저장
...
checkJSON.addEventListener("click", () => {
  ...
  console.log("obj2 확인 : ", obj2);

  // 기상청 서버에 07/12 전국 미세먼지 농도를 JSON으로 받아오기
