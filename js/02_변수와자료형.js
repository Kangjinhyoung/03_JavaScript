/* 변수 선언 */
// 메모리(RAN)에 값을 저장할 공간을 만들고 이름을 붙이는 것
// 작성법 : 변수종류 변수명;
...
/* 변수에 값을 대입 */
// 선언 된 변수(공간)에 값을 대입(집어 넣음)
// 작성법 : 변수명 = 값;
// =(등호) : 대입 연산자
// A = B -> B를 A에 대입 (오른쪽 값을 왼쪽 변수에 대입)

number1 = 10; // number1 변수에 10을 대입
number2 = 20; // number2 변수에 20을 대입
...
function varCheck() {
  ...
  // -> 나중에 이전값을 사용하고 싶은데 나도 모르게 새로운 값을 대입해 지워버리는 경우가 생긴다
}
// 2. let
...
function letCheck() {
  ...
  // Identifier 'name' has already been declared -> name 변수가 이미 선언됨 
...
/* 선수 지식
  ...
  2. 지역 변수 : 특정 지역에서 사용 가능한 변수
     1) 함수 레벨 범위
     - function 함수명() {}
       함수 정의 부분에서 {} 내부에서 var로 선언된 변수는 함수 내부 어디서든 사용 가능
     2) 블록 레벨 범위
 */
// 함수 선언
function scopeTest() {
  ...
  if(num1 == 100) {
    ...
    // 함수 {} 레벨 지역 변수
    // -> if문이 끝나도 함수가 끝난게 아니기 때문에 함수 {} 안에서 어디서든 사용 가능
  ...
  // num4 is not defiend -> num4가 정의되지 않음
...
function typeCheck() {
  ...
  /* undifiend : 정의 되지 않은 변수 / 값 대입 X 또는 존재 X */
  let undef; // undef 변수를 선언함
  ...
  // V(value) == 변수에 저장된 값
  const user = {
    id    : "user01", // const id = "user01"; 이랑 같음
